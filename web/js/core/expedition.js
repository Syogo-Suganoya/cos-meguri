...
/** 会話の取得は1ページ1回。チャット欄とプラン復元の両方が同じ結果を使う。 */
let pending = null;
...
/** 合わせを取り直す。無ければ null。控えが古ければ捨てる。 */
export async function currentAwase() {
  const id = store.awaseId();
  if (!id) return null;
  try {
    return await api(`/api/awase/${id}`);
  } catch {
    store.setAwaseId(null);
    return null;
  }
}
