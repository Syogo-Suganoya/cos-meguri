...
// ---- 画面の写し ----
//
// 大きいまま並べるとページが伸びる。一覧は小さい画像（shots/thumbs/）で、
// 選んだ1枚だけ大きく出す。大きい画像は選ばれたときに読み込む。
// 画像は docs/shots.js が利用者と同じ順に操作して撮っている（README も同じ画像を指す）。
//
// 並びはログインしていない状態（相談〜動線まで）を先に見せ、そのあとログインして
// マイページでお気に入りを確かめる、という利用者の実際の流れに合わせてある。

const SHOTS = [
  ["01-top", "トップ", "「使ってみる」はログインなしで相談へ入ります。"],
  ["02-ask", "相談", "イベント名・日付・目的地・時間と、作品名・キャラ名・出発駅・荷物。押したときに足りない欄は、その欄の真下に出ます。"],
  ["03-plan-guest-makeup", "プラン（ゲスト）: メイクの工程", "ログインしなくてもプランは見られます。☆ を押すと、ログインと登録を案内します。"],
  ["04-plan-guest-route", "プラン（ゲスト）: 動線", "行きは開始までに着き、帰りは終了後に出ます。動線も、ログインする前から見られます。"],
  ["05-signup", "アカウントを作る", "☆ の案内から来ます。ログインする前に組んだプランは、そのまま引き継ぎます。"],
  ["06-me", "マイページ", "ログインすると、押した ☆ がお気に入りに並びます。保存した時点の写しなので、組み直しても消えません。"],
];
